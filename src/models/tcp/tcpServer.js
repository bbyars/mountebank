...
var create = function (port, options) {
    var name = options.name ? util.format('tcp:%s %s', port, options.name) : 'tcp:' + port,
        ...
        server = net.createServer(function connectionListener (client) {
            ...
            logger.debug('%s connected', clientName);
            ...
            client.on('data', function (data) {
                ...
                logger.debug('%s => %j', clientName, data.toString(encoding));
                ...
                domain.run(function () {
                    stubs.resolve(request).done(function (stubResponse) {
                        ...
                        if (buffer.length > 0) {
                            logger.debug('%s <= %j', clientName, buffer.toString(encoding));
                            client.write(buffer);
                        }
                    }, errorHandler);
                });
            ...
            client.on('end', function () {
                ...
                logger.debug('%s ended connection', clientName);
            });
        });
