...
function normalizeJSON (possibleJSON) {
    try {
        ...
        return stringify(JSON.parse(possibleJSON), null, 2);
    }
    catch (e) {
...
function replaceVolatileData (text, volatileLines) {
    return volatileLines.reduce((accumulator, volatileLinePattern) => {
        ...
        if (lineIndex >= 0) {
            ...
            textLines[lineIndex] = textLines[lineIndex].replace(matches[1], '"VOLATILE"').replace(/""/g, '"');
        }
...
function normalize (text, responseElement) {
    const trimmed = (text || '').trim(),
        normalizedVolatility = replaceVolatileData(trimmed, collectVolatileLines(responseElement)),
        normalizedJSON = normalizeJSONSubstrings(normalizedVolatility);
